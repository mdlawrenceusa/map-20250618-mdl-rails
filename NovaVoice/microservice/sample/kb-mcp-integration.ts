...
import { BedrockAgentClient, CreateKnowledgeBaseCommand, CreateDataSourceCommand, StartIngestionJobCommand, GetIngestionJobCommand } from '@aws-sdk/client-bedrock-agent';
...
const bedrockAgent = new BedrockAgentClient({ region: 'us-east-1' });
const s3Client = new S3Client({ region: 'us-east-1' });

// Store KB configuration
interface KBConfig {
  knowledgeBaseId?: string;
  dataSourceId?: string;
  s3Bucket: string;
  status: 'not_configured' | 'creating' | 'ready' | 'syncing';
  lastSync?: string;
}
...
async function saveKBConfig() {
  const configPath = path.join(__dirname, '../../../kb-config.json');
  await fs.writeFile(configPath, JSON.stringify(kbConfig, null, 2));
}
...
export async function createKnowledgeBase(req: Request, res: Response) {
  try {
    ...
    const kbCommand = new CreateKnowledgeBaseCommand({
      name: 'nova-sonic-kb',
 ...
 */
export async function configureKnowledgeBase(req: Request, res: Response) {
  try {
 ...
 */
export async function getKBStatus(req: Request, res: Response) {
  try {
    const status = {
 ...
 */
function updateEstherWithKB(knowledgeBaseId: string) {
 ...
 * Express route setup
 */
export function setupKBRoutes(app: any) {
  ...
  // Simple UI endpoint
  app.get('/kb-setup', (req: Request, res: Response) => {
    res.send(`
      <!DOCTYPE html>
      <html>
      <head>
