...
const express_ws_1 = require("express-ws");
...
const simple_transcript_logger_1 = require("./simple-transcript-logger");
...
function setUpEventHandlersForChannel(session, channelId) {
    ...
    session.onEvent("textOutput", (data) => {
        if (data && data.content) {
            ...
            simple_transcript_logger_1.simpleTranscriptLogger.addText(channelId, speaker, content);
        }
    });
...
wsInstance.app.ws("/socket", (ws, req) => {
    ...
    const channelId = req.query.channel?.toString() || (0, uuid_1.v4)();
            ...
            if (clients.size === 0) {
                ...
                if (session && bedrockClient.isSessionActive(channelId)) {
                    try {
                        ...
                        await simple_transcript_logger_1.simpleTranscriptLogger.endCall(channelId);
...
app.get("/outbound/answer", (req, res) => {
    ...
    simple_transcript_logger_1.simpleTranscriptLogger.startCall(callUuid, phoneNumber);
    let ncco;
