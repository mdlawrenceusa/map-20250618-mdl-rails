...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Find the most recent leads.json part files (Spark output format)
        const sortedFiles = listResult.Contents
            .filter(obj => obj.Key.includes('/leads.json/') && obj.Key.includes('part-'))
            .sort((a, b) => b.LastModified - a.LastModified);
        
        if (sortedFiles.length === 0) {
            throw new Error('No leads.json part files found');
        }
        
        const latestFile = sortedFiles[0];
        console.log(`Processing file: ${latestFile.Key}`);
        
        // Get all part files from the same leads.json directory
        const dirPath = latestFile.Key.substring(0, latestFile.Key.lastIndexOf('part-'));
        const dirListParams = {
            Bucket: OUTPUT_BUCKET,
            Prefix: dirPath
        };
        
        const dirContents = await s3Client.send(new ListObjectsV2Command(dirListParams));
        const jsonFiles = dirContents.Contents.filter(obj => 
            obj.Key.includes('part-') && obj.Key.endsWith('.json') && !obj.Key.includes('_SUCCESS')
        );
