...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Find the most recent items.json part files (Spark output format)
        const sortedFiles = listResult.Contents
            .filter(obj => obj.Key.includes('/items.json/') && obj.Key.includes('part-'))
            .sort((a, b) => b.LastModified - a.LastModified);
        
        if (sortedFiles.length === 0) {
            throw new Error('No items.json part files found');
        }
        ...
        const fileContent = await s3Client.send(new GetObjectCommand(getParams));
        const bodyString = await fileContent.Body.transformToString();
        ...
        for (let i = 0; i < lines.length; i += BATCH_SIZE) {
            ...
            if (putRequests.length > 0) {
                try {
                    ...
                    const result = await dynamodb.send(new BatchWriteCommand(batchParams));
                    ...
                    if (result.UnprocessedItems && 
                        ...
                        while (unprocessed.length > 0 && retryCount < 3) {
                            ...
                            const retryResult = await dynamodb.send(new BatchWriteCommand({
                                RequestItems: {
